...
import { ChevronDown, Headphones, SlidersHorizontal, Upload, X } from "lucide-react";
import {
  SENSITIVITY_PRESETS,
  TIME_SCALE_PRESETS,
  MIN_SENSITIVITY_UV,
...
type PanelSection = "recording" | "review" | "events" | "audio" | "advanced";
...
export function ControlPanel({ onClose }: { onClose?: () => void }) {
  ...
  return (
    <aside className="flex h-full min-h-0 w-full flex-col bg-surface" aria-label="Review controls">
      <div className="sticky top-0 z-20 shrink-0 border-b border-border bg-surface/95 px-2 py-2 backdrop-blur">
        <div className="flex items-center justify-between gap-2 px-1 pb-2">
          <div>
            <p className="text-[0.625rem] font-semibold uppercase tracking-[0.14em] text-subtle">
              Review controls
            </p>
            <p className="mt-0.5 text-[0.6875rem] text-muted">Local session · no upload</p>
          </div>
          {onClose && (
            <Button
              size="iconSm"
        ...
        <nav className="grid grid-cols-5 gap-1" aria-label="Control sections">
          {(
            [
              ["recording", "File"],
              ["review", "View"],
            ...
            <button
              ...
              className={`h-7 rounded-sm px-1 text-[0.625rem] font-semibold tracking-wide transition-colors ${activeSection === id ? "bg-accent text-accent-fg" : "text-muted hover:bg-surface-2 hover:text-fg"}`}
            >
              {label}
            </button>
          ))}
            ...
            <section className="space-y-3 p-4">
              ...
              {recording?.header.identifierWarning && (
                <p className="text-xs text-pretty text-warn">
                  Header text may still contain identifiers. Patient fields are hidden here —
                  confirm deidentification before sharing.
                </p>
              )}
            ...
            <section className="space-y-3 p-4">
              ...
              {soundMode === "off" ? (
                <p className="text-pretty text-xs leading-5 text-muted">
                  Visual review is independent of audio. Select a sound mode in the header when you
                  want an auditory representation.
                </p>
                ...
                <>
                  <div className="rounded-md border border-border bg-bg p-3">
                    ...
                    <p className="mt-1 text-pretty text-xs leading-5 text-muted">
                      ...
                      events/s. Realtime pages are anchored in successive 10-second epochs; the
                      export uses the selected 10-second epoch. This reproduces the disclosed
                      symbolic mapping, not clinical validity.
                    </p>
                    {soundMode === "hybrid" && (
                      <p className="mt-2 text-pretty text-xs leading-5 text-warn">
                        Hybrid adds a softer second-harmonic style after mapping. Pitch and event
                ...
                <>
                  <p className="text-pretty text-xs leading-5 text-muted">
                    These mappings support listening and exploration. They have not been validated
                    for diagnosis or clinical benefit. Display filters and polarity do not alter
                    their input.
                  </p>
                  {soundMode === "experimental" ? (
                    <>
                    ...
                    <div className="rounded-md bg-bg p-3 shadow-border">
                      <p className="text-xs font-medium text-fg">
                        Contour events + pentatonic style
                      </p>
          ...
          <>
            ...
            <section className="space-y-3 p-4">
              ...
              <p className="text-xs text-pretty text-muted">
                These settings affect the visual trace only. Analysis and sonification use
                separately documented preprocessing.
              </p>
                ...
                <div className="space-y-4 border-t border-border px-3 py-3">
                  {soundMode === "evidence" || soundMode === "hybrid" ? (
                    <div className="rounded-md border border-border bg-surface p-3 text-xs leading-5 text-muted">
                      Evidence source is locked to raw Fz–Cz. Mixer gain, mute, solo, pan, and
                      audible scrubbing are disabled so they cannot silently alter the study
                      mapping.
                    </div>
                  ) : (
                    <>
                      <div className="space-y-2">
                        <div>
                          ...
                          <p className="mt-1 text-pretty text-xs text-muted">
                            S = solo, M = mute. Double-click S for an exclusive solo. Track gains
                            stay live during playback.
                          </p>
                        </div>
                        <MixerStrip />
                      </div>
