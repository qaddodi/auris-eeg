 ...
 */
export const DETERMINISTIC_SCREENING_VERSION = "deterministic-screening-0.1.0";
...
function detectorThresholds(detector: ScreeningDetector): ScreeningThreshold[] {
  switch (detector) {
    case "focal-slowing": return [{ name: "slow relative power", operator: ">=", value: 0.45, units: "relative" }, { name: "slow-to-fast ratio", operator: ">=", value: 1.4, units: "ratio" }];
    case "generalized-slowing": return [{ name: "fraction of channels with slow relative power", operator: ">=", value: 0.6, units: "fraction" }];
    case "hemispheric-asymmetry": return [{ name: "left/right total-power ratio", operator: "ratio", value: 1.8, units: "ratio" }];
    case "suppression-attenuation": return [{ name: "robust scale relative to cohort", operator: "<=", value: 0.35, units: "ratio" }, { name: "absolute RMS support", operator: "<", value: 5, units: "input units" }];
...
function evaluateDetectors(
  ...
  const focal = eeg.filter((channel) => {
    ...
    const relativeToCohort = safeRatio(slow, Math.max(cohortSlow, 0.02));
    return slow >= 0.45 && metric.spectralRatios.slowToFast >= 1.4 && relativeToCohort >= 1.35;
  });
  ...
  for (const channel of focal) {
    ...
    ], clamp(0.55 + 0.2 * boundedScore(metric.spectralRatios.slowToFast - 1.4), 0, 1), 0.15, `Relative slowing screen in ${channel.provenance.label}; focal localization is provisional and montage-dependent.`);
  }

  const generalizedFraction = eeg.length ? focal.length / eeg.length : 0;
  const generalized = eeg.length >= 2 && generalizedFraction >= 0.6;
  evaluate("generalized-slowing", generalized, eeg.length);
  if (generalized) positive("generalized-slowing", eeg, null, [{ feature: "channels_meeting_slowing_rule", value: focal.length, units: "channels", direction: "higher" }, { feature: "slow_channel_fraction", value: generalizedFraction, units: "fraction", direction: "higher" }], clamp(0.55 + generalizedFraction * 0.35, 0, 1), 0.15, "A majority of eligible EEG channels meet the deterministic slowing rule.");
